...
class WeightedDigraph(Digraph):
    ...
    def addEdge(self, edge):
        ...
        temp = [dest, (float(weight1), float(weight2))]
        self.edges[src].append(temp)
        # append 
        
    def childrenOf(self, node):
        childrens = []
        for d in self.edges[node]: # a list 
            childrens.append(d[0])
        return childrens # a list 
    def __str__(self):
        res = ''
        for k in self.edges: # each key in dictionary, which is of type Node
            #for d in self.edges[str(k)]:
             for d in self.edges[k]: # a list
                 res += str(k) + '->'+ str(d[0]) + " " + str(d[1]) + '\n'
